...
When('I should see {string} items in the cart bubble', async function(cartNumber){
    ...
    assert.equal(await DriverMethods.GetTextFromElement("className", selectors.ItemNumberInCart), cartNumber)
})
    ...
    switch(sortType){
        ...
        */
        case "Price (high to low)":
            noSortPrices = await DriverMethods.GetTextFromListOfElements("className", selectors.PriceList)
            ...
            await DriverMethods.SelectFromDropdownUsingText("className", selectors.ProductSort, sortType)
            priceAfterSortByUI = await DriverMethods.GetTextFromListOfElements("className", selectors.PriceList)
        ...
        Finally we compare both arrays if they are the same
        */
            noSortNames = await DriverMethods.GetTextFromListOfElements("className", selectors.ProductList)
            sortedNamesZtoA = await noSortNames.sort().reverse();

            await DriverMethods.SelectFromDropdownUsingText("className", selectors.ProductSort, sortType)
            namesAfterSortByUI = await DriverMethods.GetTextFromListOfElements("className", selectors.ProductList)
